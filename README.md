# Termux Hosting Setup

This project provides a Python script to set up a hosting environment in Termux that supports PHP, MySQL, HTML, CSS, JavaScript, and React.
...
1. **Install Python and dependencies**:
   - Open Termux and install Python by running:
     ```bash
     pkg install python
     ```
...
2. **Create the Python script**:
   ...
   - Save the file and exit (`Ctrl + X`, then press `Y` and `Enter`).

3. **Run the script**:
   - Execute the Python script to set up the environment:
     ```bash
     python setup_host.py
     ```

4. **Access the Hosting**:
   - Once the setup is complete, you can access the following:
     - **PHP site**: `http://localhost`
     - **React app**: `http://localhost:3000`

## Features

- **PHP Hosting**: PHP is set up with MySQL and can be accessed via Nginx.
- **MySQL Database**: MySQL is installed and can be used for your backend needs.
- **React App**: A basic React app is created and can be run on the server.
- **Nginx Configuration**: The server is configured to serve PHP files and run the React app on separate ports.

## Troubleshooting

- Ensure that Termux is up to date by running `pkg update -y` before setting up the environment.
- If the MySQL service does not start, try running `mysqld_safe &` manually.
